...
{   
    ...
    $scope.filtermyarray = function(myEvents) {
            ...
            myEvents.forEach(function(event){
                     ...
                     console.log($scope.valueDataEvent );
                     console.log($scope.valueDataEvent2);


                    console.log("my event get time" , new Date(event.start_at.__text).getTime()/ 1000);
                     console.log("my input for starting date" ,new Date($scope.valueDataEvent).getTime() / 1000);
                     console.log("my input for ending date", new Date($scope.valueDataEvent2).getTime() / 1000); 
                ...
                if ( new Date(event.start_at.__text).getTime() >= new Date($scope.valueDataEvent).getTime() && (new Date(event.start_at.__text).getTime() <= new Date($scope.valueDataEvent2).getTime())) {
                    ...
                    console.log("choosen event", event);
                    $scope.myNewEvents.push(event);
                }
            })
              console.log("myNewEvents" , $scope.myNewEvents);
    };
